...
Providers (auto-detected, first available wins):
  FREE:  ollama   — local model, zero cost
         ...
         nvidia   — NVIDIA NIM cloud,        set NVIDIA_API_KEY
...
def cmd_setup(args):
    ...
    providers = {
        ...
        "4": ("nvidia",   "NVIDIA NIM (cloud)          — needs NVIDIA_API_KEY"),
...
def cmd_providers(args):
    ...
    tier = {
        ...
        "deepseek": "cheap",      "nvidia": "NIM cloud",
...
def main():
    ...
    parser.add_argument("--provider", "-p",
                        help="Force provider: ollama / nvidia / groq / deepseek / claude / openai / grok")
    ...
    cfg = load_config()
    for env_var in ("GROQ_API_KEY", "DEEPSEEK_API_KEY", "NVIDIA_API_KEY", "ANTHROPIC_API_KEY",
